...
impl Computer {
    fn new(code: Vec<i64>) -> Self {
        ...
        x.code.extend((1..1000).map(|_| 0).collect::<Vec<i64>>());
        x
    }
    fn run(&mut self) {
        loop {
    ...
    fn less_than(&mut self) {
        ...
        if self.code[pos1 as usize] < self.code[pos2 as usize] {
            self.code[pos3 as usize] = 1;
        } else {
            self.code[pos3 as usize] = 0;
        }
        self.index += 4;
    }
    fn equals(&mut self) {
    ...
    fn jump_if_true(&mut self) {
        ...
        if self.code[pos1 as usize] != 0 {
            self.index = self.code[pos2 as usize] as usize;
        } else {
            self.index += 3;
        }
    }
    ...
    fn get_input(&mut self) {
        ...
        self.code[pos1 as usize] = val;
        self.index += 2;
    }
    fn output(&mut self) {
        let [pos1, _, _] = self.get_positions();
        self.outputs.push(self.code[pos1 as usize]);
        self.index += 2;
    }
    fn add(&mut self) {
        let [pos1, pos2, pos3] = self.get_positions();
